...
* **💳 Подписки** — трекер платных подписок через Mini App: цена в любой валюте с пересчётом, период от недели до года, статистика «в месяц / в год» в валюте чата, напоминание о списании (за 1/3/7 дней) с кнопками «Оплачено» и «Без напоминаний», пауза, пресеты популярных сервисов
...
* **⚙️ Telegram Mini App** — веб-приложение для управления API-ключами, системным промптом, напоминаниями, подписками и номерами машин
